...
func check_if_can_build(tile_pos: Vector2i, building: Dictionary) -> bool:
	...
	var tile_set_source = building.get("simple_house").get("source_id")
	
	var x_horizontal = building.get("simple_house").get("x_horizontal")
	var y_horizontal = building.get("simple_house").get("y_horizontal")
	var vertical = building.get("simple_house").get("height")
...
func place_building(tile_pos: Vector2i, building: Dictionary):
	if (check_if_can_build(tile_pos, building)):
		...
		var tile_set_source = building.simple_house.source_id
		...
		for y_atlas_place in range(vertical-1, -1, -1):
			...
			for k in range(0, y_horizontal):
				...
				tile_map_layers[tile_map_layer_index + 1].set_cell(Vector2i(x_index, y_index), 1, Vector2i(y_atlas_place_temp, y_atlas_place))
